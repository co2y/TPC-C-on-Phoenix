...
public class jTPCCConnection {
	...
	log.info("12");
        stmtNewOrderUpdateStock = dbConn.prepareStatement(
                "UPSERT INTO bmsql_stock(s_w_id,s_i_id,s_quantity,s_ytd,s_order_cnt,s_remote_cnt) " +
                        "    select s_w_id,s_i_id,?, s_ytd + ?, " +
                        "        s_order_cnt + 1, " +
                        "        s_remote_cnt + ? FROM bmsql_stock" +
                        "    WHERE s_w_id = ? AND s_i_id = ?");
	log.info("13");
        stmtNewOrderInsertOrderLine = dbConn.prepareStatement(
                "UPSERT INTO bmsql_order_line (" +
	...
	log.info("13-1");
        ...
        stmtPaymentUpdateWarehouse = dbConn.prepareStatement(
                "UPSERT INTO bmsql_warehouse(w_id,w_ytd)" +
                        "    SELECT w_id,w_ytd+? FROM bmsql_warehouse " +
                        "    WHERE w_id = ?");
        stmtPaymentUpdateDistrict = dbConn.prepareStatement(
                "UPSERT INTO bmsql_district(d_w_id,d_id,d_ytd) " +
                        "    SELECT d_w_id,d_id,d_ytd + ? FROM bmsql_district " +
                        "    WHERE d_w_id = ? AND d_id = ?");
        stmtPaymentUpdateCustomer = dbConn.prepareStatement(
                "UPSERT INTO bmsql_customer(c_w_id,c_d_id,c_id,c_balance, c_ytd_payment,c_payment_cnt) " +
                        "    SELECT  c_w_id,c_d_id,c_id,c_balance - ?, " +
                        "         c_ytd_payment + ?, " +
                        "         c_payment_cnt + 1 FROM bmsql_customer" +
                        "    WHERE c_w_id = ? AND c_d_id = ? AND c_id = ?");
        stmtPaymentUpdateCustomerWithData = dbConn.prepareStatement(
                "UPSERT INTO bmsql_customer(c_w_id,c_d_id,c_id,c_balance,c_ytd_payment,c_payment_cnt,c_data) " +
                        "    SELECT c_w_id,c_d_id,c_id,c_balance - ?, " +
                        "         c_ytd_payment + ?, " +
                        "         c_payment_cnt + 1, " +
                        "         ? FROM bmsql_customer " +
        ...
        stmtPaymentInsertHistory = dbConn.prepareStatement(
                "UPSERT INTO bmsql_history (" +
                        " hist_id,   h_c_id, h_c_d_id, h_c_w_id, h_d_id, h_w_id, " +
                        "    h_date, h_amount, h_data) " +
                        "VALUES (next value for BMSQL_HIST_ID_SEQ,?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            default:
                stmtStockLevelSelectLow = dbConn.prepareStatement(
                        "SELECT count(*) AS low_stock FROM (" +
                                ...
                                "            SELECT  ol_i_id " +
                                "                FROM bmsql_district " +
                                "                JOIN bmsql_order_line ON ol_w_id = ? " +
                                "                 AND ol_d_id = ? " +
                                "                WHERE d_w_id = ? AND d_id = ? " +
                                "                 AND ol_o_id >= d_next_o_id - 20 " +
                                "                 AND ol_o_id < d_next_o_id " +
	...
	log.info("15-1");
        stmtDeliveryBGUpdateOrder = dbConn.prepareStatement(
                "UPSERT INTO bmsql_oorder(o_w_id,o_d_id,o_id,o_carrier_id) " +
                        "   SELECT o_w_id,o_d_id,o_id,? FROM bmsql_oorder " +
                        "    WHERE o_w_id = ? AND o_d_id = ? AND o_id = ?");
        ...
        stmtDeliveryBGUpdateOrderLine = dbConn.prepareStatement(
                "UPSERT INTO bmsql_order_line(ol_w_id,ol_d_id,ol_o_id,ol_number,ol_delivery_d) " +
                        "    SELECT ol_w_id,ol_d_id,ol_o_id,ol_number,? FROM bmsql_order_line " +
                        "    WHERE ol_w_id = ? AND ol_d_id = ? AND ol_o_id = ?");

	log.info("15-2");
        stmtDeliveryBGUpdateCustomer = dbConn.prepareStatement(
                "UPSERT INTO bmsql_customer(c_w_id,c_d_id,c_id,c_balance, c_delivery_cnt) " +
                        "    SELECT c_w_id,c_d_id,c_id,c_balance + ?, " +
                        "         c_delivery_cnt + 1 FROM bmsql_customer " +
                        "    WHERE c_w_id = ? AND c_d_id = ? AND c_id = ?");
	log.info("16");
    }
